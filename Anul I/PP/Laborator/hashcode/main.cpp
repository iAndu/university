    ...
    for (int i = 0; i < c; ++i) {
        ...
        if (caches[i].cac_videos[j].nr_req != 0) {
            if (video[caches[i].cac_videos[j].id_vid] <= caches[i].cac_size) {
                ...
                for (int k = 0; k < r; ++k) {
                    if (request[k].r_vid == caches[i].cac_videos[j].id_vid) {
                            int l;
                        for (l = 0; l < endpoint[request[k].r_ep].c_latency.size(); ++l) {
                            if (endpoint[request[k].r_ep].c_latency[l].first == i) {
                                break;
                            }
                        }
                        if (l != endpoint[request[k].r_ep].c_latency.size()) {
                            for (int l = 0; l < endpoint[request[k].r_ep].c_latency.size(); ++l) {
                                if (endpoint[request[k].r_ep].c_latency[l].first != i) {
                                    for (int q = 0; q < caches[endpoint[request[k].r_ep].c_latency[l].first].cac_videos.size(); ++q) {
                                        if (caches[endpoint[request[k].r_ep].c_latency[l].first].cac_videos[q].id_vid == request[k].r_vid)
                                        {
                                            caches[endpoint[request[k].r_ep].c_latency[l].first].cac_videos[q].ponder -= caches[i].cac_videos[q].nr_req * endpoint[request[k].r_ep].c_latency[l].second * 1LL;
                                        break;
                                    }
                                    }
                                }
...
int main() {
    ofstream out("trending_today.out");
    read();
    complete_cache();
    sort_caches();
    final_caches();
